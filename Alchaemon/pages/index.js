...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch(`https://eth-testnet.g.alchemy.com/nft/v2/${alchemyApiKey}/getContractsForOwner?owner=${address}`, options)
          ...
          {contracts.map(contract => {
            return (
              <li key={contract.address}>
                <a href={`https://sepolia.etherscan.io/address/${contract.address}`} target="_blank">
                  {contract.address}
                </a>
              </li>
            )
